...
import { getPartnerDetail } from '@/api/partner';
import { followUser, unfollowUser } from '@/api/follow';
import { useRequest } from 'ahooks';
import { createHistoryRecord } from '@/api/history';
import { getImageUrl, getImageCdnUrl, getHeaderHeight } from '@/utils';
import {
  SECTION_MAP,
  typeConfigMap,
  getTransportLabel,
...
export default function TripDetail() {
  ...
  // 搭子详情平铺返回行程字段，映射为 guide 结构
  const guide = partnerData
    ? {
        id: partnerData.id,
        title: partnerData.title,
        coverImage: partnerData.cover,
        cities: [partnerData.destination].filter(Boolean),
        provinces: [],
        summary: partnerData.desc,
        members: [],
  ...
  // 分享好友：携带搭子 ID（分享本页，展示关联行程）
  useShareAppMessage(() => ({
    title: guide?.title || '发现一个好行程，一起出发吧！',
    path: `/pages/partner/view/index?id=${id}`,
    ...
    <>
      ...
      <NavBar showBack backgroundColor="white">
        ...
        <View
          ...
          onClick={() =>
            Taro.navigateTo({
              url: `/pages/accounting/list/index?targetType=partner&targetId=${id}&name=${encodeURIComponent(guide?.title || '')}`,
            })
          }
        >
