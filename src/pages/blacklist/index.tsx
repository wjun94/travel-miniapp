...
export default function BlacklistPage() {
  ...
  const handleUnblock = async (userId: string, e: any) => {
    ...
    setUnblocking(true);
    const ok = await unblockUser(userId).then(() => true).catch(() => false);
    setUnblocking(false);
    if (!ok) return;
        ...
        >
          <View className="flex items-center flex-1 min-w-0 space-x-3">
            ...
            <View className="flex-1 min-w-0">
              ...
              <Text className="text-[22px] text-stone-400 mt-0.5 block">
                用户ID: {item.userId}
              </Text>
            </View>
          </View>
          ...
          >
            <Text
              className={`text-[24px] font-medium ${isUnblocked ? 'text-stone-400' : 'text-red-500'}`}
            >
