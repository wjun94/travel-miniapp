...
export default function TravelGuideDetail() {
  ...
  const {
    ...
    error,
    refresh,
  } = useRequest(() => getTravelGuideDetail(id || ''), {
    refreshDeps: [id],
    onSuccess: (data) => {
      setCurrentDayIdx(0);
      if (data?.title) {
  ...
  const difficultyLabel =
    difficultyOptions.find((d) => d.key === guide?.difficulty)?.label ||
    guide?.difficulty ||
    '';
  const guideStats: {
    key: string;
    ...
    },
    {
      ...
      svg: WatchSvg,
      label: '建议行程天数',
      value: guide?.recommendedDays ? `${guide.recommendedDays} 天深度游` : '',
      iconClass: 'h-3.5 w-3.5 mr-6px',
    },
    {
      key: 'difficulty',
    ...
    <>
      <NavBar showBack backgroundColor="white">
        ...
        <View
          ...
          onClick={() =>
            Taro.navigateTo({
              url: `/pages/accounting/list/index?targetType=guide&targetId=${id}&name=${encodeURIComponent(guide?.title || '')}`,
            })
          }
        >
