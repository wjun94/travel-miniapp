...
export default function TripDetail() {
    ...
    const tripStats: { key: string; label: string; value: string; highlight?: boolean; svg?: string; iconClass?: string }[] = [
        { key: 'city', svg: CitySvg, label: '途经城市数', value: `${guide.cities?.length || 0} 个城市`, iconClass: 'h-4 w-4 mr-6px' },
        { key: 'days', svg: WatchSvg, label: '规划天数', value: `${days.length} 天深度游`, iconClass: 'h-3.5 w-3.5 mr-6px' },
        { key: 'mate', svg: TeamSvg, label: '出行伴侣', value: guide.members && guide.members.length > 0 ? `${guide.members.length} 人同行` : '独自探索', highlight: true, iconClass: 'h-3.5 w-3.5 mr-6px' },
        { key: 'view', svg: HotSvg, label: '浏览热度', value: `${guide.viewCount || 0} 次围观`, iconClass: 'h-3 w-3 mr-6px' },
    ];
